"""Draw and audit the PDF geometry before and after reversing only w arrows.
...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/qsl_w_lattice_mpl")
import matplotlib
matplotlib.use("Agg")
...
from quadrupolar_spin_liquid_w_reversed import (
    GEOMETRY,
    INTERNAL_A1,
    INTERNAL_A2,
    ORIGINAL_GAUGE,
...
def paper_figure_edges(reverse_w: bool) -> set[tuple]:
    ...
    for i in range(5):
        ...
        result.extend((b, a, kind) if reverse_w and kind == "w" else (a, b, kind)
                      for a, b, kind in pair)
    result.extend((lower[i], upper[i], "y") for i in range(6))
    result.extend((upper[i], lower[i + 1], "z") for i in range(5))
    result.extend((upper[i + 1], lower[i], "t") for i in range(5))
    return set(result)
...
def audit_geometry() -> dict:
    ...
    changed_keys = {key for key in ORIGINAL_GAUGE if ORIGINAL_GAUGE[key] != W_REVERSED_GAUGE[key]}
    require(changed_keys == {"u43w", "u12w"}, f"Gauge contamination: {changed_keys}")
    for key in ("u34x", "u42z", "u31z"):
        require(W_REVERSED_GAUGE[key] == +1, f"Forbidden gauge change: {key}")
    old_edges, new_edges = geometry_edges(ORIGINAL_GAUGE), geometry_edges(W_REVERSED_GAUGE)
    changed_edges = []
    for old, new in zip(old_edges, new_edges, strict=True):
        ...
        if old.kind == "w":
            require((new.start, new.end) == (old.end, old.start), "A w arrow was not reversed exactly")
            changed_edges.append({"old": asdict(old), "w_reversed": asdict(new)})
        else:
            require(old == new, "A non-w arrow changed")
    ...
    solutions = [
        ...
        if all(signs[i - 1] * ORIGINAL_GAUGE[key] * signs[j - 1] == W_REVERSED_GAUGE[key]
               for i, j, kind, key, d1, d2, shift in PAPER_BONDS)
    ]
    require(len(candidates) == 16, "Diagonal gauge search is incomplete")
    require(not solutions, "Unexpected diagonal gauge equivalence")
    old_flux, new_flux = audit_fluxes(ORIGINAL_GAUGE), audit_fluxes(W_REVERSED_GAUGE)
    for name, (_, kinds) in PAPER_PATHS.items():
        require(new_flux[name]["flux"] == (-1 if "w" in kinds else +1) * old_flux[name]["flux"],
                f"Derived flux violates w-edge parity: {name}")
    ...
    return {
        ...
        "reversed_w_edge_count": len(changed_edges),
        "non_w_edges_unchanged": True,
        ...
        "W_REVERSED_EDGES": [asdict(edge) for edge in new_edges],
        "OLD": old_flux,
        "W_REVERSED": new_flux,
...
def draw_lattice(output_path: Path, dpi: int = 300) -> None:
    ...
    fig.suptitle("Inversão exclusiva das ligações w", fontsize=17, y=.972, weight="bold", color="#1D2837")
    ...
    fig.text(.39, .058, "u₄₃,w: +1 → -1,    u₁₂,w: -1 → +1", fontsize=11.5, color=COLORS["w"])
...
def main() -> None:
    ...
    output = args.output_dir / "qsl_lattice_w_reversed.png"
    draw_lattice(output)
    current = metadata()
    for kind in ("script_sha256", "source_sha256", "local_source_sha256"):
        require(result["metadata"][kind] == current[kind], f"Sources changed while rendering geometry: {kind}")
    audit_file = args.output_dir / "geometry_w_audit.json"
    ...
    print(json.dumps({"visible_edges": result["visible_edge_count"],
                      "reversed_w_edges": result["reversed_w_edge_count"],
